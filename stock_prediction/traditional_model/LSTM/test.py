...
def plot_direction_accuracy(y_true, y_pred, dates):
    ...
    plt.tight_layout()
    plt.savefig('direction_accuracy.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    return accuracy

def test_model(model_path='best_model.h5', 
    ...
    dummy_array_train_pred[:, 1] = train_pred.reshape(-1)  # 使用reshape(-1)将预测结果展平
    ...
    dummy_array_test_pred[:, 1] = test_pred.reshape(-1)  # 使用reshape(-1)将预测结果展平
